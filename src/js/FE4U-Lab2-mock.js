...
function id() {
  const sybmols = 'abcdefghijklmnopqrstuvwxyz0123456789';
  let id = '';
  for (let i = 0; i < 32; i++) {
    id += sybmols[Math.floor(Math.random() * sybmols.length)];
  }
  return id;
}
...
function getUsers(users) {
  ...
  users.forEach(user => {
    ...
    if (user.id.name && user.id.value) {
      newUser.id = `${user.id.name}${user.id.value}`;
    } else {
      newUser.id = id();
    }
...
function getUniqueUsers(firstArray, secondArray) {
  const uniqueUsers = [...new Set([...firstArray, ...secondArray])];
  return uniqueUsers;
}
...
function validateUser(user) {
  const fields = ['full_name', 'note', 'state', 'city', 'country'];
  for (const field of fields) {
    if (typeof (user[field]) !== 'string' || user[field][0] !== user[field][0].toUpperCase()) {
      return false;
    }
  };

  if (typeof (user.gender) !== 'string' || user.gender[0] !== user.gender[0].toLowerCase()) return false;
...
function sortUsers(users, param, ascending) {
  const result = [...users]
  // Визначаємо змінну ascendingModifier для визначення напрямку сортування
  // Якщо ascending === true, то ascendingModifier = 1 (висхідний порядок), інакше - ascendingModifier = -1 (спадаючий порядок)
  ...
  result.sort((user1, user2) => {
    // Порівнюємо значення параметра param між user1 та user2
    if (user1[param] === user2[param]) return 0;

    // Якщо значення param відсутнє для user1, то user1 розміщується вгорі (або внизу, залежно від ascending)
    if (!user1[param]) return -1 * ascendingModifier;

    // Якщо значення param відсутнє для user2, то user2 розміщується вгорі (або внизу, залежно від ascending)
    if (!user2[param]) return 1 * ascendingModifier;

    // В інших випадках порівнюємо значення param для user1 та user2 і визначаємо порядок сортування
    // Використовуємо ascendingModifier, щоб вказати напрямок сортування (висхідний або спадаючий)
    return user1[param] > user2[param] ? 1 * ascendingModifier : -1 * ascendingModifier; 
  });

  // Повертаємо відсортований масив користувачів
  return result;
}
...
function findUsers(users, param) {
  ...
  if (param.note) {
      return users.filter(user => {
          return user.note && user.note.includes(param.note);
    })
  }
  if (param.name) {
      return users.filter(user => {
          return user.full_name && user.full_name.includes(param.name);
    })
  }
  return [];
}
...
function findPercentage(users, param) {
  const percentage = ((findUsers(users, param).length / users.length) * 100).toFixed(2);
  return percentage;
}
...
//task1
const firstArray = getUsers(randomUserMock);
const allUniqueUsers = getUniqueUsers(firstArray,additionalUsers);

console.log(allUniqueUsers.length);
console.log(allUniqueUsers[0]);

//task2
//console.log(validateUser(allUniqueUsers[2]));

//task3
//console.log(filterUsers(allUniqueUsers, {country:'Norway', gender:'male'}));

//task4
//console.log(sortUsers(allUniqueUsers, 'age', false));

//task5
//console.log(findUsers(allUniqueUsers, {name: 'Matilda'}));

//task6
//console.log(findPercentage(allUniqueUsers, {age: 71}));
